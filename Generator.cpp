...
Generator::Generator(const Generator &toCopy)
{
    ID = toCopy.ID;
    name = toCopy.name;
    race = toCopy.race;
    charClass = toCopy.charClass;
    alignment = toCopy.alignment;
    level = toCopy.level;
    stats = toCopy.stats;
    hp = toCopy.hp;
    personality = toCopy.personality;
}
...
void displayGenerator(Generator display)
{
    ...
    cout << "Alignment: " << belief.at(display.getsetAlign()[0]) << belief.at(display.getsetAlign()[1]) << "\n";
...
int rollStat()
{
    ...
    int roll1 = (arc4random() % 6) + 1;
    int roll2 = (arc4random() % 6) + 1;
    int roll3 = (arc4random() % 6) + 1;
    int roll4 = (arc4random() % 6) + 1;
    int sum;
...
{
    return  arc4random() % availableRaces.size();
}

int Generator::generateClass()
{
    return arc4random() % availableClasses.size();
}

array<int, 2> Generator::generateAlign()
{
    int bel = abs((int)arc4random() % 3);
    int mor = abs((int)arc4random() % 3);
...
QString Generator::generatePersonailty()
{
    ...
    int traitNum = arc4random() % numLines + 1;
    ...
    traitNum = arc4random() % numLines + 1;
...
int saveGenerator(Generator toSave)
{
    ...
    if (success)
    {
        ...
        logError(gDbErr, error, getlogDir());
    ...
    if (returnCode != SQLITE_OK)
    {
        ...
        error.append("): ");
        error.append(QString::fromStdString(sqlite3_errmsg(db)));
        logError(genFailPrep, error, getlogDir());
    ...
    else
    {
        ...
        if( returnCode != SQLITE_OK )
        {
            ...
            error.append(", ");
            error.append(QString::number(returnCode));
            logError(genFailBind, error, getlogDir());
...
int sqlToGen(void* data, int argc, char** argv, char** azColName)
{
    ...
    try {
        ...
        logError(GenCreateErr, error, getlogDir());
        return -1;
    }
...
int readGenerators()
{
    ...
    if( success )
    {
        QString error = "Cannot open database: ";
        error.append(QString::fromStdString(sqlite3_errmsg(db)));
        logError(genOpenErr, error, getlogDir());
        //printf("Cannot open database %s\n", sqlite3_errmsg(db));
        return -1;
    }
    ...
    if( returnCode != SQLITE_OK )
    {
        ...
        logError(genFailRead, error, getlogDir());
        //printf("SQL error: %s\n", zErrMsg);
