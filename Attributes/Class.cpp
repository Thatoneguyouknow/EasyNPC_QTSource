...
Class::Class(const Class &toCopy)
{
    classID = toCopy.classID;
    name = toCopy.name;
    hitDie = toCopy.hitDie;
    statPriority = toCopy.statPriority;
}
...
int sqlToClass(void* data, int argc, char** argv, char** azColName) {
    ...
    try {
        ...
        logError(cCreateErr, error, getlogDir());
        return -1;
    }
    ...
    if(newClass.getClassID() != id)
    {
        ...
        error.append(QString::number(id));
        logError(claIdMismatch, error, getlogDir());
...
int readClasses()
{
    ...
    if( success )
    {
        ...
        logError(claOpenErr, error, getlogDir());
    ...
    if( returnCode != SQLITE_OK )
    {
        ...
        logError(claFailRead, error, getlogDir());
        //printf("SQL error: %s\n", zErrMsg);
...
int saveClass(Class toSave)
{
    ...
    if (success)
    {
        ...
        QString error = "Cannot open database: ";
        error.append(QString::fromStdString(sqlite3_errmsg(db)));
        logError(cDbErr, error, getlogDir());
    ...
    if (returnCode != SQLITE_OK)
    {
        ...
        error.append("): ");
        error.append(QString::fromStdString(sqlite3_errmsg(db)));
        logError(claFailPrep, error, getlogDir());
    ...
    else
    {
        ...
        if( returnCode != SQLITE_OK )
        {
            ...
            error.append(", ");
            error.append(QString::number(returnCode));
            logError(claFailBind, error, getlogDir());
...
{
    cout << toDisplay.getsetName() << "\n";
    cout << toDisplay.getsetHitDie() << "\n";
}
