...
void generateBaseRaces()
{
    ...
    int hAsi1 = (arc4random() % 6) + 1;
    int hAsi2 = (arc4random() % 6) + 1;
...
Race::Race(const Race &toCopy)
{
    raceIdentifier = toCopy.raceIdentifier;
    raceName = toCopy.raceName;
    asiStatPrimary = toCopy.asiStatPrimary;
    asiStatSecondary = toCopy.asiStatSecondary;
    asiValPrimary = toCopy.asiValPrimary;
    asiValSecondary = toCopy.asiValSecondary;
    ageMax = toCopy.ageMax;
    ageMin = toCopy.ageMin;
    nameType = toCopy.nameType;
}
...
{    
    ...
    if(newRace.getIdentifier() != ID)
    {
        ...
        error.append(QString::number(ID));
        logError(raceIdMismatch, error, getlogDir());
...
int readRaces()
{
    ...
    if( success )
    {
        ...
        logError(raceOpenErr, error, getlogDir());
    ...
    if( returnCode != SQLITE_OK )
    {
        ...
        logError(raceFailRead, error, getlogDir());
        //printf("SQL error: %s\n", zErrMsg);
...
int saveRace(Race toSave)
{
    ...
    if( success )
    {
        ...
        logError(rDbErr, error, getlogDir());
        //printf("Cannot open database %s\n", sqlite3_errmsg(db));
        return -1;
    }
    ...
    if(returnCode != SQLITE_OK )
    {
        ...
        error.append(": ");
        error.append(sqlite3_errmsg(db));
        logError(raceFailPrep, error, getlogDir());
    ...
    else
    {
        ...
        if( returnCode != SQLITE_OK )
        {
            ...
            error.append(", ");
            error.append(QString::number(returnCode));
            logError(raceFailBind, error, getlogDir());
